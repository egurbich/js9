...
function findElementGreaterThan(arr, num) {
  ...
  if (!Array.isArray(arr)) {
    console.log("Переданий параметр не є масивом.");
    return undefined;
  }
...
function findLastEvenNumber(arr) {
  ...
  const lastEvenIndex = arr.findLast(num => num % 2 === 0);

  if (lastEvenIndex !== undefined) {
...
function getArrayKeys(arr) {
  ...
  const keysIterator = Object.keys(arr);

  const arrayKeys = Array.from(keysIterator);
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  const sum = lastNElements.reduceRight((acc, curr) => acc + curr, 0);
